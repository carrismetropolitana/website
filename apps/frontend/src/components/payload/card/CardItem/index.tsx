...
export function CardItem({ card, colors, isFirstChild, isLastChild }: { card: PayloadCardItem, colors: CustomCSSProperties, isFirstChild: boolean, isLastChild: boolean }) {
	...
	return (
		<div className={styles.container} data-is-first={isFirstChild} data-is-last={isLastChild} data-open={isOpen} style={colors}>
				...
				<div className={styles.innerWrapper}>
					...
					{imageUrl ? (
						<div className={styles.contentLottie}>
							<img alt={titleText} src={imageUrl} style={{ height: 'auto', maxHeight: 220, objectFit: 'contain', width: '100%' }} />
						</div>
					) : null}
					...
					{descriptionText ? (
						<p className={styles.contentDescription}>
							{descriptionLines.map((line, idx) => (
								<span key={`${idx}-${line}`}>{line} {idx < descriptionLines.length - 1 ? <br /> : null}</span>
							))}
						</p>
					) : null}
