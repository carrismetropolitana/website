...
export function AlertsSection() {
	...
	const alertsActiveTodayAndTomorrow = useMemo(() => {
		...
		const sortedAlerts = alertsContext.data.alerts.sort((a, b) => new Date(b.active_period_start_date).getTime() - new Date(a.active_period_start_date).getTime());
		// Filter alerts to only include alerts that start today or tomorrow
		const filteredAlerts = sortedAlerts.filter((alert) => {
			const today = DateTime.now().startOf('day').toJSDate();
			const alertDate = new Date(alert.active_period_start_date);
