	...
	return (
		<div className={styles.container}>
			{allEncmGroupedByMunicipality &&
				allEncmGroupedByMunicipality.map(group => <div key={group.id} className={styles.groupWrapper}>
					<div className={styles.groupTitle}>
						<h2>{group.name}</h2>
					</div>
					<div key={group.id} className={styles.grid}>
						{group.encm && group.encm.map(encm => <FrontendEncmItem key={encm.id} encmData={encm} selectedEncmId={selectedEncmId} onSelectEncmId={onSelectEncmId} />)}
					</div>
				</div>)}
		</div>
	);
