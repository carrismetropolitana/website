...
export default function FrontendLinesToolbarFiltersMunicipality() {
  ...
  return (
    <div className={styles.container}>
      <Select aria-label={t('label')} clearable data={allMunicipalitiesDataFormatted} nothingFoundMessage={t('no_results')} onChange={handleSelectMunicipality} placeholder={t('placeholder')} radius="sm" searchable size="md" value={FrontendLinesContext.entities.municipality?.id || null} w="100%" />
    </div>
  )
